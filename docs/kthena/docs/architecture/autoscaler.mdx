...
Kthena Autoscaler provides three scaling granularities: **Homogeneous Instances Autoscale**, **Heterogeneous Instances Autoscale**, and **PD Disaggregation Autoscale**.
...
### PD Disaggregation Autoscale

When a model is deployed with dedicated prefill and decode roles, **PD Disaggregation Autoscale** scales both roles in one control loop.

This mode first collects metrics from both roles and merges metrics with the
same name into one combined signal. The merged signal is passed through the
same recommendation and correction flow used by regular autoscaling:

1. `RecommendedInstancesAlgorithm` calculates a recommended total replica count
   from the policy metric targets, current total replicas, total min/max bounds,
   tolerance, and unready role pods.
2. Panic mode and scale-up/scale-down behavior are applied by
   `CorrectedInstancesAlgorithm`.
3. The corrected result is used as `N_total`.

`N_total` is therefore the desired total number of prefill plus decode replicas
for the current reconcile cycle:

$$
N_{total} = N_{prefill} + N_{decode}
$$

The total lower and upper bounds are derived from both roles:

$$
N_{min} = prefill.minReplicas + decode.minReplicas,\quad
N_{max} = prefill.maxReplicas + decode.maxReplicas
$$

After `N_total` is calculated, the autoscaler splits total replicas into
prefill/decode replicas based on `prefillDecodeRatio`.

If `prefillDecodeRatio` is configured (for example `1:2`), role replicas are split using:

$$
N_{prefill} = \lceil N_{total} \cdot \frac{P}{P + D} \rceil, \quad N_{decode} = N_{total} - N_{prefill}
$$

Each role is then clamped by its own bounds:

- prefill: `[prefillRole.minReplicas, prefillRole.maxReplicas]`
- decode: `[decodeRole.minReplicas, decodeRole.maxReplicas]`

If clamping on one role creates residual capacity, autoscaler reassigns remaining replicas to the other role (within bounds) to preserve the total capacity target.

`prefillDecodeRatio` is a target distribution hint, not a hard guarantee for
all cases. The final effective split can differ from the configured ratio when:

- the computed `N_total` is too small to match the ratio exactly,
- one role reaches its `minReplicas` or `maxReplicas`,
- the ratio is omitted, in which case the current role replica distribution is
  reused when possible and falls back to `1:1` for a new target.

The effective split is reported in status as `effectiveRatio`, together with
the last reconciled prefill and decode replica counts. `prefillDecodeRatio` can
be updated at runtime. The new ratio is applied in the next reconcile cycle
without changing autoscaling policy definitions.
...
spec:
  policyRef:
    name: llama3-autoscaling-policy
