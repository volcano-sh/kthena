...
This mode first aggregates metrics from both roles into one combined signal and computes a total desired replica count. Then it splits total replicas into prefill/decode replicas based on `prefillDecodeRatio`.
...
`prefillDecodeRatio` can be updated at runtime. The new ratio is applied in the next reconcile cycle without changing autoscaling policy definitions.
