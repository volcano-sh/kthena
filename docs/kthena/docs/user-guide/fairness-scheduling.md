...
| `FAIRNESS_VTC_IDLE_TTL`        | How long an idle user/model entry is retained before eviction | `30m`         | Only applies when `FAIRNESS_MODE=vtc`. Active entries are never evicted. Non-positive or unparsable values fall back to the default |
