...
- **`vtc`**: tracks a cumulative Virtual Token Counter per `(user, model)` pair, based on the VTC algorithm from the [S-LoRA fairness paper](https://arxiv.org/abs/2401.00588). Instead of a time window, each user's counter only grows with usage, and the queue prioritizes the lowest counter. To keep an idle user from being unfairly penalized by everyone else's usage catching up, a user's counter is lifted to the current minimum among active users the moment they re-enter the queue after being idle.
