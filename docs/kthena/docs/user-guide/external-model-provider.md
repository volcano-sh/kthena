...
Errors generated by the Router itself use the same response shape as the internal `ModelServer` path: the response body is a JSON string containing the safe error message. These errors are classified as `provider_discovery`, `request_protocol`, `provider_request_build`, `response_forwarding`, or `external_provider_proxy` as appropriate. When an upstream response has already been received, its status code, headers, body, and streaming envelope are forwarded without translating the provider protocol. OpenAI-compatible upstream errors therefore retain their OpenAI `error` envelope, while Anthropic upstream errors retain the Anthropic `type`/`error` envelope.
