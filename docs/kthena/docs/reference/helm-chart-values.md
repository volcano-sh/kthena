...
| networking.kthenaRouter.sessionBoost.maxWait | string | `"30s"` | Maximum time a request may wait in the session-boost queue before it is<br/> rejected with HTTP 429. Only effective when `waitRejectEnabled` is `true`.<br/> Keep it below the general queue timeout so 429 fires before a 504. |
| networking.kthenaRouter.sessionBoost.waitRejectEnabled | bool | `false` | Reject requests that wait longer than `maxWait` in the session-boost<br/> queue with HTTP 429 instead of waiting for the general queue timeout.<br/> Disabled by default. |
