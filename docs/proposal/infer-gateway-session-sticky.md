...
- Session sticky for **PD disaggregation** mode. Because `ModelRoute` and `ModelServer` are separate objects with independent create/update timing, cross-object target resolution is not reliable at admission time. Instead, the router evaluates resolved targets at runtime; if any selected target is PD (`WorkloadSelector.PDGroup` present), sticky is bypassed for that request/route and a warning is logged.
- **Future work**: PD-specific session sticky semantics are planned and will be introduced in a follow-up proposal.
...
1. **Session key extraction**  
  ...
  - **JWTClaim**: read a string claim from the **Authorization Bearer** token using the **same JWKS and validation path** as the router’s JWT authenticator. If JWT auth is disabled, the token is missing/invalid, or the claim is absent, the value is treated as empty for that source; to avoid silent misconfiguration, a route that configures any `JWTClaim` source must also enable JWT authentication on that route (otherwise admission is rejected).
...
4. **Routing**  
   - **`sessionSticky` set**, **non-empty** session key: **Get** mapping; if the mapped Pod name appears in the current filtered pod list, the scheduler **pins** selection to that Pod (still running score plugins on that candidate).  
   ...
   - On failover, rebinding must use Redis CAS (or SET NX + version) to prevent concurrent replicas from splitting one session.
   - `sessionSticky` **nil/omitted** or **empty** session key after extraction: existing scheduling only.  
   - **PD targets are not eligible for sticky configuration by validation**, so routing never has to do per-request sticky disable for PD.
...
Lookup semantics: **Header** names are case-insensitive per HTTP; **Cookie** names are case-sensitive and must match exactly.
...
- **Webhook (`ModelRoute`)**: when **`spec.sessionSticky` is non-nil**, **`sources` must be non-empty**; **`sessionAffinitySeconds`**, if set, must be **≥ 1**. (No cross-object PD-target rejection at admission due to object lifecycle ordering.)
...
| E2E-SS-12 | **PD target present at runtime**: one `ModelRoute` fans out to multiple `ModelServer`s and any resolved target has `PDGroup` set, while `sessionSticky` is non-null. | Request is served with sticky **bypassed** (normal scheduling path), and router emits a warning log for the PD-sticky incompatibility. |
