...
| `kthena_router_prefix_cache_entries`           | Gauge    | —       | Current total cached hash entries, summed across all per-pod caches at scrape time. Bounded by `(#pods with entries) × maxHashCacheSize`; once every per-pod cache is full the value plateaus (1-for-1 eviction) and changes only as pods are added or deleted. |
