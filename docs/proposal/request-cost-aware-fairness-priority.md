...
7. Predict the actual generated output token length before model execution.

### Proposal

This PR is intended to carry the proposal only. The router and datastore
implementation should be added after the proposal is reviewed and approved.
...
The proposal does not estimate the actual generated output length. Before a
request is executed, the router cannot know how many output tokens the backend
will produce. Instead, it uses the requested output token budget as a
conservative upper-bound estimate for enqueue ordering.
...
If neither output-budget field is present, the enqueue-time request estimate is
input-only. After the request completes, the existing token accounting still
records actual output tokens from the response and updates historical fairness
usage in the sliding window. This keeps the enqueue estimate separate from
...
When these variables are unset, the enqueue-time calculation should use the same
defaults as the token tracker. The current tracker defaults are `1.0` for input
tokens and `2.0` for output tokens. Keeping these defaults aligned avoids unit
mismatches between the enqueue-time estimate and the historical token usage.
...
If the priority queue refreshes or rebuilds priorities while requests are
