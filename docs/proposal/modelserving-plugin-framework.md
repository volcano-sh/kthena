...
The built-in `headless-service` plugin uses `OnPodCreate` to inject `ENTRY_ADDRESS`, set the Pod hostname and Service subdomain for Entry and Worker Pods, and create the initial Service. Its selector covers every Pod in the Role replica so per-Pod records are published beneath the Headless Service domain. `OnRoleSync` recovers the Service; `OnRoleDelete` performs cleanup. Without the plugin, neither the Service nor these generated DNS settings are added. The Service is optional auxiliary state and is not used to decide whether a Role or ServingGroup exists.
