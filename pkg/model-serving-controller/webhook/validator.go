...
func (v *ModelServingValidator) validateModelServing(modelServing *workloadv1alpha1.ModelServing) (bool, string) {
	...
	allErrs = append(allErrs, validateTemplateMetadataLabels(modelServing)...)
...
func validateTemplateMetadataLabels(ms *workloadv1alpha1.ModelServing) field.ErrorList {
	var allErrs field.ErrorList
	for i, role := range ms.Spec.Template.Roles {
		allErrs = append(allErrs, validateReservedTemplateLabels(
			role.EntryTemplate.Metadata,
			field.NewPath("spec").Child("template").Child("roles").Index(i).Child("entryTemplate").Child("metadata").Child("labels"),
		)...)
		if role.WorkerTemplate != nil {
			allErrs = append(allErrs, validateReservedTemplateLabels(
				role.WorkerTemplate.Metadata,
				field.NewPath("spec").Child("template").Child("roles").Index(i).Child("workerTemplate").Child("metadata").Child("labels"),
			)...)
		}
	}
	return allErrs
...
func validateReservedTemplateLabels(metadata *workloadv1alpha1.Metadata, labelsPath *field.Path) field.ErrorList {
	if metadata == nil {
		return nil
	}
	var allErrs field.ErrorList
