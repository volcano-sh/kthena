...
func (v *ModelServingValidator) Handle(w http.ResponseWriter, r *http.Request) {
	...
	if allowed && admissionReview.Request.Operation == admissionv1.Update {
		...
		if len(admissionReview.Request.OldObject.Raw) == 0 {
			http.Error(w, "update admission request has no old ModelServing object", http.StatusBadRequest)
			return
		}
		...
		allowed, reason = validateModelServingUpdate(&oldModelServing, modelServing)
	}
...
// validateModelServingUpdate validates fields that depend on the previous
// version of a ModelServing.
func validateModelServingUpdate(oldModelServing, newModelServing *workloadv1alpha1.ModelServing) (bool, string) {
	oldRoleNames := roleNames(oldModelServing)
	newRoleNames := roleNames(newModelServing)
	if sameRoleNames(oldRoleNames, newRoleNames) {
		return true, ""
	}
	...
	err := field.Invalid(
		...
		newRoleNames,
		fmt.Sprintf("role names are immutable; existing role names are %v", oldRoleNames),
	)
	return false, fmt.Sprintf("validation failed:\n  - %s", err.Error())
}

func sameRoleNames(oldRoleNames, newRoleNames []string) bool {
	if len(oldRoleNames) != len(newRoleNames) {
		return false
	}
	existing := make(map[string]int, len(oldRoleNames))
	for _, name := range oldRoleNames {
		existing[name]++
	}
...
func roleNames(ms *workloadv1alpha1.ModelServing) []string {
	names := make([]string, 0, len(ms.Spec.Template.Roles))
