...
type Store interface {
	...
	// user/model is about to enter the fairness queue. Must be paired with
	// exactly one OnRequestFinish call.
	OnRequestStart(userId, modelName string)
	// OnRequestFinish notifies the token tracker that a request started via
	// OnRequestStart is done: served after admission, or dropped from the
	// queue by timeout, cancellation or an enqueue failure.
