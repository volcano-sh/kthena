...
func (h *HTTPConnector) ProxyEPD(c *gin.Context, reqBody map[string]interface{}, encodeAddr, prefillAddr, decodeAddr string, hooks *OnFlightHooks) (int, error) {
	...
	encodeRequest := buildPrefillRequest(c.Request, encodeBody)

	if encodeRequest == nil || prefillRequest == nil || decodeRequest == nil {
		return 0, fmt.Errorf("failed to build EPD request(s)")
	}
	...
	err := h.encode(encodeRequest, encodeAddr)
