...
func TestValidateModelRoute(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectValid:    false,
			expectedReason: "validation failed:\n  - spec.sessionSticky.sources: Required value: sources must be non-empty when sessionSticky is set",
		},
		{
			...
			expectValid:    false,
			expectedReason: "validation failed:\n  - spec.sessionSticky.sessionAffinitySeconds: Invalid value: 0: must be >= 1",
		},
	}
