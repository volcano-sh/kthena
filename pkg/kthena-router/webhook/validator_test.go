...
func TestValidateModelRoute(t *testing.T) {
	tests := []struct {
		...
		{
			name: "invalid model route - targetModel with empty modelServerName",
			modelRoute: &networkingv1alpha1.ModelRoute{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "networking.serving.volcano.sh/v1alpha1",
					Kind:       "ModelRoute",
			...
			expectValid:    false,
			expectedReason: "validation failed:   - spec.rules[0].targetModels[0].modelServerName: Required value: modelServerName must not be empty",
		},
		{
			name: "invalid model route - nil target model entry",
			modelRoute: &networkingv1alpha1.ModelRoute{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "networking.serving.volcano.sh/v1alpha1",
					Kind:       "ModelRoute",
			...
			expectValid:    false,
			expectedReason: "validation failed:   - spec.rules[0].targetModels[0]: Invalid value: null: target model must not be nil",
		},
		{
			name: "invalid model route - mixed valid and invalid targetModels",
			modelRoute: &networkingv1alpha1.ModelRoute{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "networking.serving.volcano.sh/v1alpha1",
					Kind:       "ModelRoute",
			...
			expectValid:    false,
			expectedReason: "validation failed:   - spec.rules[0].targetModels[1].modelServerName: Required value: modelServerName must not be empty  - spec.rules[0].targetModels[2]: Invalid value: null: target model must not be nil",
		},
		{
