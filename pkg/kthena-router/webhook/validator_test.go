...
func TestValidateModelServer(t *testing.T) {
	tests := []struct {
		...
		{
			name: "invalid model server - missing workload selector",
			modelServer: &networkingv1alpha1.ModelServer{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "networking.serving.volcano.sh/v1alpha1",
					Kind:       "ModelServer",
			...
			expectValid:    false,
			expectedReason: "validation failed:   - spec.workloadSelector: Required value: workloadSelector must be specified",
		},
		{
			name: "invalid model server - empty match labels",
			modelServer: &networkingv1alpha1.ModelServer{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "networking.serving.volcano.sh/v1alpha1",
					Kind:       "ModelServer",
