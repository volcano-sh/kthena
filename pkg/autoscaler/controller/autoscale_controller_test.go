...
func TestPDDisaggregatedHighLoad_then_DoScale_expect_RoleReplicasAndStatus(t *testing.T) {
	...
	binding.Spec.PDDisaggregatedTarget.PrefillRole.MetricEndpoint = workload.PodMetricSource{Uri: u.Path, Port: port}
	binding.Spec.PDDisaggregatedTarget.DecodeRole.MetricEndpoint = workload.PodMetricSource{Uri: u.Path, Port: port}
