...
type RolloutStrategy struct {
	...
	// rolling update settings on individual Roles are invalid.
	// RoleRollingUpdate uses the rolling update configuration on each Role and
	// does not allow rolloutStrategy.rollingUpdateConfiguration.
	//
	...
	// It must not be set maxUnavailable when type is RoleRollingUpdate;
	// +optional
...
type RollingUpdateConfiguration struct {
	...
	// Partition is the resource ordinal below which replicas are protected
	// from updates. Resources with ordinals greater than or equal to Partition
	// are updated. It can be an absolute number or a percentage.
	// A percentage is rounded up. Defaults to 0.
	// +kubebuilder:validation:XIntOrString
	// +optional
