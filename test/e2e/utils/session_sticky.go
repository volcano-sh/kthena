...
const (
	...
	// SessionStickyModelServerFile is the ModelServer template that enables session sticky.
	SessionStickyModelServerFile = "ModelServer-session-sticky.yaml"
...
// SessionStickyRegisterModelServerCleanup deletes the ModelServer when the test finishes.
func SessionStickyRegisterModelServerCleanup(t *testing.T, kthenaClient *clientset.Clientset, testNamespace string, ms *networkingv1alpha1.ModelServer) {
	t.Helper()
	if ms == nil {
		return
	}
	t.Cleanup(func() {
		_ = kthenaClient.NetworkingV1alpha1().ModelServers(testNamespace).Delete(
			context.Background(), ms.Name, metav1.DeleteOptions{})
	})
}

// SessionStickyCreateModelServer loads the sticky ModelServer template, applies customize, then creates it.
func SessionStickyCreateModelServer(
	t *testing.T,
	ctx context.Context,
	kthenaClient *clientset.Clientset,
	testdataDir, testNamespace string,
	customize func(*networkingv1alpha1.ModelServer),
) *networkingv1alpha1.ModelServer {
	t.Helper()
	ms := LoadYAMLFromFile[networkingv1alpha1.ModelServer](filepath.Join(testdataDir, SessionStickyModelServerFile))
	ms.Namespace = testNamespace
	if customize != nil {
		customize(ms)
	}
	out, err := kthenaClient.NetworkingV1alpha1().ModelServers(testNamespace).Create(ctx, ms, metav1.CreateOptions{})
	require.NoError(t, err)
	return out
}
